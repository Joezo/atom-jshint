...
module.exports = AtomJshint = (function(){
  ...
  AtomJsHint.prototype.watchConfig = function(file){
    var self = this;
    this.fileWatcher = fs.watch(file, { persistent: false }, function onConfigChanged() {
      // A new watch will be set up in loadConfig, and it might find a file that didn't exist last time
      if (self.fileWatcher) self.fileWatcher.close();
      self.loadConfig();
    });
  };

  AtomJsHint.prototype.loadConfig = function(){
    var watchConfig = atom.config.get('atom-jshint.watchConfig');
    if( fs.existsSync(atom.project.path + '/package.json') ){
      if (watchConfig) this.watchConfig(atom.project.path + '/package.json');
    ...
    while ( dirs.length >= 1 ) {
      ...
      if( fs.existsSync(configPath) ){
        ...
        if (watchConfig) this.watchConfig(configPath);
