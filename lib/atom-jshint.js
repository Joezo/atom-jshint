  ...
  AtomJsHint.prototype.updateGutter = function(errors){
    ...
    if (activeView) {
      ...
      decorations.forEach(function(dec) {
            dec.destroy();
            decorations.splice(decorations.indexOf(dec), 1);
          });
      errors.forEach(function(error){
          var range = [[error.line-1, 0], [error.line-1, 0]];
          var marker = activeView.editor.markBufferRange(range);
          var decoration = activeView.editor.decorateMarker(marker, {type: 'line', class: 'atom-jshint-error'});
